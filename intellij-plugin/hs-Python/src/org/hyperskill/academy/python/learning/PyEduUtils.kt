...
fun installRequiredPackages(project: Project, sdk: Sdk) {
  ...
  try {
    // Install using pip install -r requirements.txt directly
    // This supports URL-based requirements that the plugin API doesn't handle
    LOG.warn("PyEduUtils: Installing from requirements.txt, SDK path = ${sdk.homePath}")
    runWithModalProgressBlocking(project, EduPythonBundle.message("installing.requirements.progress")) {
      ...
      reportSequentialProgress(2) { reporter ->
        reporter.itemStep("Preparing to install $packageCount package(s) from requirements.txt") {
        ...
        reporter.itemStep("Running: pip install -r requirements.txt ($packageNames)") {
          ...
          if (SystemInfo.isMac) {
            ...
            commandLine.environment["CPPFLAGS"] = "-I/opt/homebrew/opt/openblas/include -I/opt/homebrew/opt/libomp/include -Xpreprocessor -fopenmp"
          ...
          LOG.warn("PyEduUtils: pip install output: exitCode=${output.exitCode}, stdout=${output.stdout.take(500)}, stderr=${output.stderr.take(500)}")
  ...
  LOG.warn("PyEduUtils: Installation finished, installationSuccessful=$installationSuccessful")
  if (installationSuccessful) {
    LOG.warn("PyEduUtils: Showing success notification")
