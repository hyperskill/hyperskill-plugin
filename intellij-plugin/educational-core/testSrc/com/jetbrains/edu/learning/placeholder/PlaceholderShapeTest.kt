...
class PlaceholderShapeTest : EduTestCase() {
  ...
  private fun checkRectangular(text: String, start: LogicalPosition, end: LogicalPosition) {
    val expected = listOf(
      LogicalPositionInLine(start.line, start.column),
  ...
  data class LogicalPositionInLine(
  ...
  private fun getMessage(expected: List<Point>, actual: List<Point>) =
    "Actual path:   ${actual.toHumanReadableString()}\nExpected path: ${expected.toHumanReadableString()}"
}
