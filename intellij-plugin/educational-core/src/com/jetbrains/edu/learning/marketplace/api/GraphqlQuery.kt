...
object GraphqlQuery {
  ...
  fun searchById(courseId: Int, searchPrivate: Boolean) =
    if (searchPrivate) {
      GeneratorUtils.getInternalTemplateText(
        "marketplace.qraphql.loadPrivateCourseById.txt",
        mapOf("courseId" to courseId)
      )
    }
    else {
      GeneratorUtils.getInternalTemplateText(
        "marketplace.qraphql.loadPublicCourseById.txt",
        mapOf("courseId" to courseId)
      )
    }
   ...
   */
  fun lastUpdatesList(courseIds: List<Int>) = GeneratorUtils.getInternalTemplateText(
    "marketplace.qraphql.courseUpdatesById.txt",
