 ...
 * IntelliJ Platform expects module JARs named as {moduleName}.jar where moduleName matches
 * the module XML descriptor filename inside the JAR. Gradle plugin creates JARs as
 * {pluginName}.{gradleModuleName}.jar which may not match the XML descriptor name.
 * This workaround reads the XML descriptor from each JAR and renames the JAR accordingly.
 */
fun renameModuleJarsToMatchDescriptors(modulesDir: java.io.File) {
  if (!modulesDir.exists() || !modulesDir.isDirectory) return

  modulesDir.listFiles()?.filter { it.extension == "jar" }?.forEach { moduleJar ->
    val zipFile = java.util.zip.ZipFile(moduleJar)
    try {
      // Find the module descriptor XML (should be at root level, not in META-INF)
      var descriptorName: String? = null
      val entries = zipFile.entries()
      while (entries.hasMoreElements()) {
        val entryName = entries.nextElement().name
        if (entryName.endsWith(".xml") && !entryName.contains("/")) {
          descriptorName = entryName
          break
        }
      }
      if (descriptorName != null) {
        val expectedJarName = descriptorName.replace(".xml", ".jar")
        if (moduleJar.name != expectedJarName) {
          zipFile.close()
          moduleJar.renameTo(java.io.File(modulesDir, expectedJarName))
        }
      }
    }
    finally {
